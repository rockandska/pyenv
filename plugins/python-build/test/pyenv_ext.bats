...
_setup() {
  ...
  export TMP_FIXTURES="$HOME/fixtures"
...
run_inline_definition_with_name() {
  ...
  local definition="${HOME}/${definition_name}"
  cat > "$definition"
...
@test "apply built-in python patch before building" {
  ...
  TMPDIR="$HOME" install_tmp_fixture definitions/vanilla-python < /dev/null
  assert_success

  assert_build_log <<OUT
patch -p0 --force -i $HOME/python-patch.XXX
Python-3.6.2: CPPFLAGS="-I${HOME}/install/include" LDFLAGS="-L${HOME}/install/lib -Wl,-rpath,${HOME}/install/lib"
...
@test "allow custom make install target" {
  ...
  PYTHON_MAKE_INSTALL_TARGET="altinstall" TMPDIR="$HOME" install_tmp_fixture definitions/vanilla-python < /dev/null
  assert_success
...
OUT
  ...
  PYTHON_MAKE_INSTALL_TARGET="" TMPDIR="$HOME" run_inline_definition <<OUT
...
OUT
  ...
  PYTHON_MAKE_INSTALL_TARGET="altinstall" TMPDIR="$HOME" run_inline_definition <<OUT
build_package_ensurepip
OUT
  assert_success
...
@test "python3-config" {
  ...
  TMPDIR="$HOME" run_inline_definition <<OUT
...
@test "enable framework" {
  ...
  PYTHON_CONFIGURE_OPTS="--enable-framework" TMPDIR="$HOME" run_inline_definition <<OUT
...
OUT
  assert_success
  assert_output <<EOS
PYTHON_CONFIGURE_OPTS_ARRAY=(--libdir=${HOME}/install/lib --enable-framework=${HOME}/install/Library/Frameworks)
...
@test "enable universalsdk" {
  ...
  PYTHON_CONFIGURE_OPTS="--enable-universalsdk" TMPDIR="$HOME" run_inline_definition <<OUT
echo "PYTHON_CONFIGURE_OPTS_ARRAY=(\${PYTHON_CONFIGURE_OPTS_ARRAY[@]})"
echo "PYTHON_CONFIGURE_OPTS=(\${PYTHON_CONFIGURE_OPTS})"
echo "CONFIGURE_OPTS=(\${CONFIGURE_OPTS})"
OUT
  assert_success
  assert_output <<EOS
PYTHON_CONFIGURE_OPTS_ARRAY=(--enable-shared --libdir=${HOME}/install/lib --enable-universalsdk=/)
PYTHON_CONFIGURE_OPTS=()
CONFIGURE_OPTS=()
EOS
}
...
OUT
  assert_success
  assert_output <<EOS
PYTHON_CONFIGURE_OPTS_ARRAY=(--enable-shared --libdir=${HOME}/install/lib --enable-universalsdk=/ --with-universal-archs=universal2)
PYTHON_CONFIGURE_OPTS=()
CONFIGURE_OPTS=()
EOS
}
...
@test "enable universalsdk with explicit archs argument" {
  ...
  PYTHON_CONFIGURE_OPTS="--enable-universalsdk --with-universal-archs=foo" TMPDIR="$HOME" run_inline_definition <<OUT
echo "PYTHON_CONFIGURE_OPTS_ARRAY=(\${PYTHON_CONFIGURE_OPTS_ARRAY[@]})"
OUT
  assert_success
  assert_output <<EOS
...
@test "enable custom unicode configuration" {
  ...
  PYTHON_CONFIGURE_OPTS="--enable-unicode=ucs2" TMPDIR="$HOME" install_tmp_fixture definitions/vanilla-python < /dev/null
  assert_success
...
@test "use custom MACOSX_DEPLOYMENT_TARGET if defined" {
  ...
  MACOSX_DEPLOYMENT_TARGET="10.4" TMPDIR="$HOME" run_inline_definition <<OUT
echo "\${MACOSX_DEPLOYMENT_TARGET}"
OUT
  assert_success
  assert_output "10.4"
